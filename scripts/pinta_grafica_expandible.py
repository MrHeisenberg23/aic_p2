...
if len(sys.argv) != 2:
	...
	if(os.path.isfile(directorio)):
		...
		for i in range(contador):
			ax.scatter(tiempos[i], energias[i], c=colors[i])
			ax.plot(tiempos[i], energias[i], c=colors[i], label=str(porcentaje_ceros[i]) + "%")
		ax.grid()
		ax.set_xlabel('Time (t)')
		ax.set_ylabel('Energy (J)')
		ax.set_title(str(hz_usado) + "MHz: Evolution Time/Energy")
		ax.legend(loc="upper left", title="Percent of zeros")
		plt.show()
